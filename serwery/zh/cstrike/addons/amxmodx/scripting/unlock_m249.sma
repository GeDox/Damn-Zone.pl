...
new const PLUGIN[] = "Robogun"
new const VERSION[] = "2.1"
new const AUTHOR[] = "CLLlAgOB (Jakemajster edit)"
...
bool:frstCLIP[33],g_MaxPlayers,g_guns_eventids_bitsum,bool:haswhpnnmg[33],
Float:g_lastShot[33], Float:g_nextSound[33], g_plAction[33],bool:g_fix_punchangle[33],
bool:canfire[33],oneround,g_normal_trace[33],DMGMG,bool:user_bot[33],
...
new P_MODEL[] = "models/gunxpmod/robogun/p_robogun.mdl"
new V_MODEL[] = "models/gunxpmod/robogun/v_robogun.mdl"
new W_MODEL[] = "models/gunxpmod/robogun/w_robogun.mdl"

new m_SOUND[][] = {"DFr/robogun/hw_shoot1.wav", "DFr/robogun/hw_spin.wav", "DFr/robogun/hw_spinup.wav", "DFr/robogun/hw_spindown.wav"}
...
public plugin_init() {
	...
	register_gxm_item("Robogun (M249)", "+Aim +Damage +Rate +Recoil ( M249 )", 300)
	register_dictionary("minigun.txt")
	clipstart = 	register_cvar("amx_ammo_mini","2000")
...
public fwd_CmdStart(id, uc_handle, seed)
{
	...
	if(!canfire[id] || !has_minigun[id]) return FMRES_HANDLED
...
public event_curweapon(id){
	if(!is_connected[id] || user_bot[id]) return;	
...
public ammo_hud(id) {
	if(!delayhud[id]) {
		...
		set_hudmessage(247, 34, 7, 0.7 , 0.7, 0, 0.1, 0.1,0.1)
...
public on_drop( id ) {
	...
	if( get_user_weapon(id) == CSW_M249 ) {
		client_print( id, print_chat, "Robogun by Jakemajster" )
		return PLUGIN_HANDLED
	}
	return PLUGIN_CONTINUE
